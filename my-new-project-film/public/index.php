...
if (isset($_GET['page']) && $_GET['page'] === 'export' && isset($_GET['action'])) {
    ...
    switch ($action) {
        ...
        case 'excel_movie': $exportController->excelMovieReviews(); exit;  // ← ДОБАВИТЬ
        case 'word_top10': $exportController->wordTop10(); exit;
        case 'word_reviews': $exportController->wordReviews(); exit;  // ← ДОБАВИТЬ
        case 'word_users': $exportController->wordUsersStats(); exit;  // ← ДОБАВИТЬ
...
if (isset($_GET['page']) && $_GET['page'] === 'export' && isset($_GET['action'])) {
    ...
    switch ($action) {
        case 'excel_top10': $exportController->excelTop10(); exit;
        case 'excel_all_reviews': $exportController->excelAllReviews(); exit;
        case 'excel_users_stats': $exportController->excelUsersStats(); exit;
        case 'excel_movie_reviews': $exportController->excelMovieReviews(); exit;
        case 'word_top10': $exportController->wordTop10(); exit;
        case 'word_all_reviews': $exportController->wordAllReviews(); exit;
        case 'word_users_stats': $exportController->wordUsersStats(); exit;
        case 'word_movie_reviews': $exportController->wordMovieReviews(); exit;
    }
}
// ===== ОБРАБОТКА AJAX =====
...
<body>
    <div class="header">
        ...
        <a href="?page=report_all_reviews" class="<?= $currentPage === 'report_all_reviews' ? 'active' : '' ?>">💬 Все отзывы</a>
        <a href="?page=report_user_stats" class="<?= $currentPage === 'report_user_stats' ? 'active' : '' ?>">👥 Статистика пользователей</a>
        <a href="?page=report_movie_reviews" class="<?= $currentPage === 'report_movie_reviews' ? 'active' : '' ?>">🎬 Отзывы по фильму</a>
    <?php endif; ?>
    ...
    <div class="container">
        ...
        switch ($page) {
            ...
            case 'login':
                ...
                break;
                // ===== ДОБАВИТЬ ЭТИ ТРИ НОВЫХ МАРШРУТА =====
    case 'report_all_reviews':
        if (isset($_SESSION['user_id'])) {
            $reportController = new Controllers\ReportController($pdo);
            $stmt = $pdo->query("
            ...
            require __DIR__ . '/../src/Views/report_all_reviews.php';
        } else {
            echo '<div class="alert alert-error">🔒 Доступно только для авторизованных</div>';
        }
        break;
    
    case 'report_user_stats':
        if (isset($_SESSION['user_id'])) {
            $reportController = new Controllers\ReportController($pdo);
            $stmt = $pdo->query("
            ...
            require __DIR__ . '/../src/Views/report_user_stats.php';
        } else {
            echo '<div class="alert alert-error">🔒 Доступно только для авторизованных</div>';
        }
        break;
    
    case 'report_movie_reviews':
        if (isset($_SESSION['user_id'])) {
            $movieId = (int)($_GET['id'] ?? 0);
            
            // Получаем все фильмы для селекта
            $stmt = $pdo->query("SELECT id, title, year FROM movies ORDER BY title");
            ...
            require __DIR__ . '/../src/Views/report_movie_reviews.php';
        } else {
            echo '<div class="alert alert-error">🔒 Доступно только для авторизованных</div>';
        }
        break;
            ...
            // ===== ОТЧЁТЫ (внутри switch!) =====
            case 'report_top10':
                if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
                    $reportController = new Controllers\ReportController($pdo);
                    $top10 = $reportController->top10();
                    require __DIR__ . '/../src/Views/admin/report_top10.php';
                }
                break;
                // ===== ПОКАЗ ОТЧЁТОВ (на сайте) =====
case 'report_top10':
    if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
        $reportController = new Controllers\ReportController($pdo);
        $top10 = $reportController->top10();
...
case 'report_all_reviews':
    if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
        $reportController = new Controllers\ReportController($pdo);
        $reviews = $reportController->getAllReviews();
        require __DIR__ . '/../src/Views/admin/report_user_activity.php';
    }
    break;
...
case 'report_user_stats':
    if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
        $reportController = new Controllers\ReportController($pdo);
        $userStats = $reportController->getUserStats();
        require __DIR__ . '/../src/Views/admin/report_user_stats.php';
    }
    break;
...
case 'report_actors':
    if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
        $reportController = new Controllers\ReportController($pdo);
        $actors = $reportController->actors();
        require __DIR__ . '/../src/Views/admin/report_actors.php';
    }
    break;
case 'report_all_reviews':
    if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
        $reportController = new Controllers\ReportController($pdo);
        $reviews = $reportController->getAllReviews();
        require __DIR__ . '/../src/Views/admin/report_all_reviews.php';
    }
    break;
    case 'all_reviews':
    if (isset($_SESSION['user_id'])) {
        $reportController = new Controllers\ReportController($pdo);
        $reviews = $reportController->getAllReviewsForDisplay();
        require __DIR__ . '/../src/Views/all_reviews.php';
    } else {
        echo '<div class="alert alert-error">🔒 Доступно только для авторизованных</div>';
    }
    break;
...
case 'user_stats':
    if (isset($_SESSION['user_id'])) {
        $reportController = new Controllers\ReportController($pdo);
        $userStats = $reportController->getUserStatsForDisplay();
        require __DIR__ . '/../src/Views/user_stats.php';
    } else {
        echo '<div class="alert alert-error">🔒 Доступно только для авторизованных</div>';
    }
    break;
...
case 'movie_reviews':
    if (isset($_SESSION['user_id'])) {
        $movieId = (int)($_GET['id'] ?? 0);
        $reportController = new Controllers\ReportController($pdo);
        $movieReviews = $reportController->getMovieReviewsForDisplay($movieId);
        $movies = \Models\Movie::getAll($pdo);
        require __DIR__ . '/../src/Views/movie_reviews.php';
    } else {
        echo '<div class="alert alert-error">🔒 Доступно только для авторизованных</div>';
    }
    break;
...
case 'export_excel':
    if (isset($_GET['action']) && isset($_SESSION['user_id'])) {
        $reportController = new Controllers\ReportController($pdo);
        $reportController->exportExcel($_GET['action']);
    }
    break;
...
case 'export_word':
    if (isset($_GET['action']) && isset($_SESSION['user_id'])) {
        $reportController = new Controllers\ReportController($pdo);
        $reportController->exportWord($_GET['action']);
    }
    break;
            case 'report_actors':
                if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
            ...
            case 'report_user_activity':
                ...
                $reviews = $reportController->userActivity($userId, $startDate, $endDate);
                $stats = $reportController->userStats($userId);
                require __DIR__ . '/../src/Views/admin/report_user_activity.php';
                break;
                
            case 'report_export':
                $reportController = new Controllers\ReportController($pdo);
                $reportController->exportExcel($_GET['type'] ?? 'top10');
                break;
